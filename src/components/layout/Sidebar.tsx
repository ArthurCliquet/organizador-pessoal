...
export function Sidebar() {
  ...
  return (
    <nav className="hidden md:flex flex-col md:w-20 lg:w-56 shrink-0 h-full bg-surface border-r border-surface-border z-10">
      <div className="flex flex-col items-center gap-1 justify-center lg:flex-row lg:items-center lg:justify-start lg:gap-2.5 h-16 shrink-0 px-3 lg:px-5 border-b border-surface-border">
        <NockMark size={22} />
        <NockWordmark size={15} className="hidden lg:inline" />
      </div>
      ...
      <div className="flex-1 flex flex-col gap-1 px-2 lg:px-3 py-4 overflow-y-auto scrollbar-thin">
        {links.map(({ to, label, icon: Icon }) => (
          <NavLink
            ...
            title={label}
            className={({ isActive }) =>
              `flex items-center md:justify-center lg:justify-start gap-3 rounded-sm px-3 py-2.5 text-[13px] font-medium transition-colors duration-150 ${
                isActive ? 'bg-primary-dim text-primary-bright' : 'text-app-muted hover:bg-surface-2 hover:text-app-text'
              }`
            }
          >
      ...
      <div className="shrink-0 px-2 lg:px-3 py-3 border-t border-surface-border flex flex-col gap-2">
        <div className="hidden lg:flex">
          <ThemeToggle />
        </div>
        <div className="flex flex-col items-center gap-2 lg:flex-row lg:items-center lg:gap-2.5 rounded-sm bg-surface-2 p-2 lg:p-2.5">
